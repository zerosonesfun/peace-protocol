...
add_action('wp_ajax_peaceprotocol_ban_user', 'peaceprotocol_ban_user_callback');
...
add_filter('user_row_actions', function($actions, $user) {
    ...
    $banned_users = get_option('peaceprotocol_banned_users', array());
...
add_action('admin_footer-users.php', function() {
    ...
    ?>
    <div id="peace-ban-modal-bg">
        <div id="peace-ban-modal">
            ...
            <form method="post" id="peace-ban-form">
                ...
                <div class="peace-ban-reason-container">
                    <label for="peace-ban-reason"><?php esc_html_e('Reason for ban:', 'peace-protocol'); ?></label>
                    <textarea id="peace-ban-reason" name="peace_ban_reason" rows="4" placeholder="<?php esc_attr_e('Please provide a reason for banning this user...', 'peace-protocol'); ?>"></textarea>
                </div>
    ...
    <!-- Ban users styles and scripts are now handled by enqueue-assets.php -->
    <?php
});
...
<body>
    <div class="banned-message">
        ...
        if ($user_id) {
            $ban_reasons = get_option('peaceprotocol_ban_reasons', array());
    ...
    This is a security-critical user banning page that must function independently.
...
function peaceprotocol_ban_user_callback() {
    if (!current_user_can('manage_options')) {
    ...
    if (!wp_verify_nonce($nonce, 'peaceprotocol_ban_user')) {
    ...
    if ($ban_action === 'ban') {
        if (!in_array($user_id, $banned_users)) {
            ...
            update_option('peaceprotocol_banned_users', $banned_users);
            update_option('peaceprotocol_ban_reasons', $ban_reasons);
            ...
            if (in_array($user_id, $banned_users)) {
                ...
                update_option('peaceprotocol_banned_users', array_values($banned_users));
                update_option('peaceprotocol_ban_reasons', $ban_reasons);
...
function peaceprotocol_is_user_banned() {
...
function peaceprotocol_is_user_id_banned($user_id) {
    $banned_users = get_option('peaceprotocol_banned_users', array());
...
add_action('rest_api_init', function() {
    ...
    add_filter('rest_pre_dispatch', function($result, $server, $request) {
        if (peaceprotocol_is_user_banned()) {
...
add_action('wp_ajax_peaceprotocol_send_peace', function() {
    if (peaceprotocol_is_user_banned()) {
        wp_send_json_error('You are banned from sending peace', 403);
    }
}, 1);

add_action('wp_ajax_peaceprotocol_generate_code', function() {
    if (peaceprotocol_is_user_banned()) {
        wp_send_json_error('You are banned from generating codes', 403);
    }
}, 1);

add_action('wp_ajax_peaceprotocol_exchange_code', function() {
    if (peaceprotocol_is_user_banned()) {
        wp_send_json_error('You are banned from exchanging codes', 403);
    }
}, 1);

add_action('wp_ajax_peaceprotocol_federated_exchange', function() {
    if (peaceprotocol_is_user_banned()) {
        wp_send_json_error('You are banned from federated exchange', 403);
    }
}, 1);

add_action('wp_ajax_peaceprotocol_validate_token', function() {
    if (peaceprotocol_is_user_banned()) {
        wp_send_json_error('You are banned from validating tokens', 403);
    }
}, 1);

add_action('wp_ajax_peaceprotocol_federated_login', function() {
    if (peaceprotocol_is_user_banned()) {
        wp_send_json_error('You are banned from federated login', 403);
    }
}, 1);

add_action('wp_ajax_peaceprotocol_complete_auth', function() {
    if (peaceprotocol_is_user_banned()) {
