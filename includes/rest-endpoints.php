...
add_action('template_redirect', function () {
    ...
    if (isset($_GET['peace_get_token']) && $_GET['peace_get_token'] == '1' && isset($_GET['return_site']) && isset($_GET['state'])) {
        ...
        if (current_user_can('manage_options') && get_option('peace_tokens')) {
            ...
            error_log('Peace Protocol: Redirecting to: ' . $redirect);
                    ...
                    if (xhr.status === 200) {
                        try {
                            ...
                            if (data && data.success) {
                                // Token is valid, redirect back with it
                                statusEl.innerHTML = '<p>Token validated! Redirecting...</p>';
                                
                                // Subscribe to the return site's feed (same as PHP path)
                                ...
                                var redirect = <?php echo json_encode($return_site); ?> + (<?php echo json_encode(strpos($return_site, '?') !== false ? '&' : '?'); ?>) + 'peace_authorization_code=' + encodeURIComponent(token) + '&peace_federated_site=' + encodeURIComponent(siteUrl) + '&peace_federated_state=' + encodeURIComponent(<?php echo json_encode($state); ?>);
                    ...
                    if (xhr.status === 200) {
                        try {
                            ...
                            if (data && data.success && data.data && data.data.redirect_url) {
                                ...
                                console.log('Peace Protocol: Missing redirect_url in response');
...
add_action('rest_api_init', function () {
    ...
    register_rest_route('peace-protocol/v1', '/federated-exchange', [
        ...
        'callback' => function ($request) {
            ...
            // Use the new option names
            $codes = get_option('peace_protocol_codes', array());
            error_log('Peace Protocol REST: Available codes: ' . print_r($codes, true));
            
            if (!isset($codes[$code])) {
                error_log('Peace Protocol REST: Code not found');
                return new WP_Error('invalid_code', 'Invalid code', ['status' => 403]);
            }
            
            $code_data = $codes[$code];
            error_log('Peace Protocol REST: Code data: ' . print_r($code_data, true));
            
            if ($code_data['expires'] < time()) {
                error_log('Peace Protocol REST: Code expired');
                unset($codes[$code]);
                update_option('peace_protocol_codes', $codes);
                return new WP_Error('invalid_code', 'Code expired', ['status' => 403]);
            }
            ...
            error_log('Peace Protocol REST: Exchanged code for token: ' . $token . ' for site: ' . $code_data['site_url']);
            error_log('Peace Protocol REST: Federated identities after exchange: ' . print_r($federated_identities, true));
...
function peace_protocol_validate_token($token) {
    ...
    // Check current site tokens
    $tokens = get_option('peace_protocol_tokens', array());
    error_log('Peace Protocol: Current site tokens: ' . print_r($tokens, true));
    
    if (isset($tokens[$token])) {
        $token_data = $tokens[$token];
        error_log('Peace Protocol: Token found in current site tokens: ' . print_r($token_data, true));
        
        if ($token_data['expires'] < time()) {
            error_log('Peace Protocol: Token expired');
            unset($tokens[$token]);
            update_option('peace_protocol_tokens', $tokens);
            return false;
        }
        
        error_log('Peace Protocol: Token valid for current site');
        return array(
...
add_action('wp_ajax_peace_protocol_complete_auth', function() {
    ...
    if (!is_user_logged_in() || !current_user_can('manage_options')) {
        error_log('Peace Protocol: User not authorized - logged in: ' . (is_user_logged_in() ? 'yes' : 'no') . ', can manage options: ' . (current_user_can('manage_options') ? 'yes' : 'no'));
    ...
    error_log('Peace Protocol: Generated auth code: ' . $auth_code);
    ...
    $separator = strpos($return_site, '?') !== false ? '&' : '?';
    $redirect_url .= $separator . 'peace_authorization_code=' . urlencode($auth_code) . '&peace_federated_site=' . urlencode(get_site_url()) . '&peace_federated_state=' . urlencode($state);
