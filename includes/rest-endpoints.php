...
function peace_protocol_validate_token($token) {
    ...
    if (in_array($token, $tokens, true)) {
        error_log('Peace Protocol: Token found in current site tokens');
        return array(
            'site_url' => get_site_url(),
            'token' => $token
        );
    ...
    // Check if it's a substring match (in case of encoding issues)
    foreach ($tokens as $index => $stored_token) {
        error_log('Peace Protocol: Comparing token ' . $index . ': ' . substr($stored_token, 0, 8) . '... vs ' . substr($token, 0, 8) . '...');
        if ($stored_token === $token) {
            error_log('Peace Protocol: Exact match found at index ' . $index);
            return array(
                'site_url' => get_site_url(),
                'token' => $token
            );
...
function peace_protocol_ajax_federated_login() {
    ...
    if ($token) {
        ...
        if ($user) {
            ...
            // Add JavaScript to set up localStorage identity
            ?>
            <script>
            // Set up localStorage identity for the federated user
            (function() {
...
add_action('wp_footer', function() {
    ...
    if (isset($_SESSION['peace_show_modal_after_login']) && $_SESSION['peace_show_modal_after_login']) {
        ...
        <script>
        (function() {
            console.log('[Peace Protocol] Session flag detected, showing peace modal');
